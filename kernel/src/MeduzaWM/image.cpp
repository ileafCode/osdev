...
void Image::draw(int x, int y)
{
    ...
    for (int yy = 0; yy < this->height; yy++)
    {
        ...
        {
            if (*(this->imgPtr + (yy * this->width + xx)) != 0xFFFF00FF)
                GlobalRenderer->PutPixDB(xx + x, yy + y, *(this->imgPtr + (yy * this->width + xx)));
        }
    }
}
...
void Image::draw(int x, int y, int w, int h)
{
    ...
    double xr = (double)this->width / (double)w;
    double yr = (double)this->height / (double)h;
    ...
    for (int yy = 0; yy < h; yy++)
    {
        for (int xx = 0; xx < w; xx++)
        {
            int xoff = (int)(xx * xr);
            int yoff = (int)(yy * yr);
            if (*(this->imgPtr + (yoff * this->width + xoff)) != 0xFFFF00FF)
                GlobalRenderer->PutPixDB(xx + x, yy + y, *(this->imgPtr + (yoff * this->width + xoff)));
        }
    }
}